...
init: function(){
  ...
  if($('.site-nav__link--main').length > 0) {
    $('.site-nav__link--main').on('mouseover', menu.navOver).on('mouseout', menu.navOut)
  }
  
	if($('.page_link').length > 0) {
		$('.page_link').on('click', menu.toggleSiteNav);
		 
	}
},

navOver: function(){
  $('.site-nav').addClass('nav-over');
},

navOut: function(){
  $('.site-nav').removeClass('nav-over');
},
